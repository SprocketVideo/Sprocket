...
| Delivery presets (built-in + save your own; a saved preset also restores the Encoding choice) | ExportPresetStore.cs; UserExportPresets.cs | export/exporting.md#save-and-reuse-presets | 🟡 stale — a saved preset now also captures Encoding (Software / Hardware if available); the guide's step 1 list omits it |
