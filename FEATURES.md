...
| Black & White conversion: 8-hue channel mixer, optical colour filter, film tone response + grain, toning / split-toning, vignette, preset library (filters, film stocks, tonings, cinematic looks) | EffectCatalog.cs `builtin.blackwhite`; Sprocket.Render/Effects/BlackWhiteEffect.cs; plan/features/black-and-white.md | — (phases 1–2/5 shipped; docs land with phase 5) | 🟡 partial (phase 2/5: conversion core — 8-hue mixer, optical filter, brightness/contrast/toe/shoulder, dry-wet Mix — plus procedural grain (per-frame or static) and vignette; toning and the preset library still to come) |
